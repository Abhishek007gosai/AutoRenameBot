...
class Database:
    ...
    def new_user(self, id):
        return dict(
            ...
            caption=None,
            metadata=True,
            metadata_code="Telegram : @EternalsHelplineBot",
    ...
    async def get_title(self, user_id):
        user = await self.col.find_one({'_id': int(user_id)})
        return user.get('title', 'Animes_Eternals')
    ...
    async def get_author(self, user_id):
        user = await self.col.find_one({'_id': int(user_id)})
        return user.get('author', 'Animes_Eternals')
    ...
    async def get_audio(self, user_id):
        user = await self.col.find_one({'_id': int(user_id)})
        return user.get('audio', 'Animes_Eternals')
    ...
    async def get_video(self, user_id):
        user = await self.col.find_one({'_id': int(user_id)})
        return user.get('video', 'Animes_Eternals')
